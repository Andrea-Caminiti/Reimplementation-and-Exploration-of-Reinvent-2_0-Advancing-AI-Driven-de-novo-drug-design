...
class Reinforcement:
    def __init__(self,
        ...
        self.stats = []
        self.optimizer = torch.optim.Adam(self.agent.network.parameters(), lr=lr)
    ...
    def run(self, steps: int, out_file: TextIOWrapper, csv_folder_path: str, name: str):
        ...
        for step in tqdm(range(steps), desc=f'Run', leave=False):
            ...
            if torch.cuda.is_available():
                score = score.cuda()
            loss = self.augment_loss(p_likelihood, score)
            loss, a_likelihood = self.buffer_filter(self.agent, loss, a_likelihood, p_likelihood, self.sigma, smiles, score)
        ...
        out_file.write(f'Average Score: {np.mean(score)}\n')
    ...
    def augment_loss(self, prior_likelihood: torch.FloatTensor, agent_likelihood: torch.FloatTensor, score: torch.autograd.Variable):
            if type(prior_likelihood) != torch.FloatTensor:
                prior_likelihood = torch.from_numpy(prior_likelihood)
            if type(agent_likelihood) != torch.FloatTensor:
                agent_likelihood = torch.from_numpy(agent_likelihood)
            if type(score) != torch.FloatTensor:
                score = torch.from_numpy(score)

            augmented = prior_likelihood + self.sigma * score
            return torch.pow((augmented - agent_likelihood), 2)
